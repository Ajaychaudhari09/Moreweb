...
export default function BlogCard({ post }: BlogCardProps) {
  ...
  return (
    <article className="card group flex flex-col overflow-hidden relative h-full">
      {/* Thumbnail (CLS-safe) */}
      {post.image ? (
      ...
      <div className="flex flex-col flex-1 p-5">
        ...
        <h3 className="card-title brand-link transition-colors line-clamp-2 mb-2 text-lg md:text-xl">
          <Link href={href} className="after:absolute after:inset-0 focus:outline-none" aria-label={`Read ${post.title}`}>
